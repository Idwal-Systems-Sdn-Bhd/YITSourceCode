    ...
    public class AkPOController : Microsoft.AspNetCore.Mvc.Controller
    {
        ...
        public async Task<IActionResult> Create(AkPO akPO, string syscode)
        {
            ...
            if (_cart.AkPOObjek != null && _cart.AkPOObjek.Count() > 0)
            {
                ...
                {
                    if (_unitOfWork.DKonfigKelulusanRepo.IsPersonAvailable(EnJenisModulKelulusan.Penilaian, EnKategoriKelulusan.Pelulus, item.JKWPTJBahagianId, akPO.Jumlah) == false)
                    {
        ...
        public async Task<IActionResult> Edit(int id, AkPO akPO, string? fullName, string syscode)
        {
            ...
            if (_cart.AkPOObjek != null && _cart.AkPOObjek.Count() > 0)
            {
                foreach (var item in _cart.AkPOObjek)
                {
                    if (_unitOfWork.DKonfigKelulusanRepo.IsPersonAvailable(EnJenisModulKelulusan.Penilaian, EnKategoriKelulusan.Pengesah, item.JKWPTJBahagianId, akPO.Jumlah) == false)
                    {
