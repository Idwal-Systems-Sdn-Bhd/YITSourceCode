        ...
        public async Task<IActionResult> Create(AkInden akInden, string syscode)
        {
            ...
            if (_cart.AkIndenObjek != null && _cart.AkIndenObjek.Count() > 0)
            {
                foreach (var item in _cart.AkIndenObjek)
                {
                    ...
                    if (_unitOfWork.DKonfigKelulusanRepo.IsPersonAvailable(EnJenisModulKelulusan.Penilaian, EnKategoriKelulusan.Pelulus, jKWPtjBahagian.JBahagianId, akInden.Jumlah) == false)
                    {
        ...
        public async Task<IActionResult> Edit(int id, AkInden akInden, string? fullName, string syscode)
        {
            ...
            if (_cart.AkIndenObjek != null && _cart.AkIndenObjek.Count() > 0)
            {
                foreach (var item in _cart.AkIndenObjek)
                {
                    ...
                    if (_unitOfWork.DKonfigKelulusanRepo.IsPersonAvailable(EnJenisModulKelulusan.Penilaian, EnKategoriKelulusan.Pengesah, jKWPtjBahagian.JBahagianId, akInden.Jumlah) == false)
                    {
