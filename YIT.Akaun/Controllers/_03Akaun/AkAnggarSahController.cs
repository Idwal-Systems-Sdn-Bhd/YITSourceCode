...
{
    [Authorize]
    public class AkAnggarSahController : Microsoft.AspNetCore.Mvc.Controller
    {
            ...
            string password)
        {
            ...
            if (dKonfigKelulusanId != null)
            {
                ...
                if (_unitOfWork.DKonfigKelulusanRepo.IsValidUser((int)dKonfigKelulusanId, password, EnJenisModulKelulusan.AnggaranHasil, EnKategoriKelulusan.Pengesah) == false)
                {
                ...
                {
                    akAnggar = _unitOfWork.AkAnggarRepo.GetResultsByDPekerjaIdFromDKonfigKelulusan(searchString, date1, date2, searchColumn, EnStatusBorang.None, (int)dKonfigKelulusanId, EnKategoriKelulusan.Pengesah, EnJenisModulKelulusan.AnggaranHasil);
        ...
        public async Task<IActionResult> Sah(int id, int dKonfigKelulusanId, string syscode)
        {
            ...
            if (akAnggar != null)
            {
                ...
                _unitOfWork.AkAnggarRepo.Sah(id, dKonfigKelulusanId, user?.UserName ?? "");

                _appLog.Insert("Posting", "Mengesah " + akAnggar.NoRujukan ?? "" + "; pengesahId: " + dKonfigKelulusanId.ToString(), akAnggar.NoRujukan ?? "", id, akAnggar.Jumlah, pekerjaId, modul, syscode, namamodul, user);
        ...
        private void PopulateFormFields(string searchString, string password, string searchDate1, string searchDate2)
        {
            ...
            ViewBag.DKonfigKelulusan = _unitOfWork.DKonfigKelulusanRepo.GetResultsByCategoryGroupByDPekerja(EnKategoriKelulusan.Pengesah, EnJenisModulKelulusan.AnggaranHasil);
        }
