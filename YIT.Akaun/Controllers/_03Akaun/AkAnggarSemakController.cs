...
{
    [Authorize]
    public class AkAnggarSemakController : Microsoft.AspNetCore.Mvc.Controller
    {
        public const string modul = Modules.kodSemakAbWaran;
        public const string namamodul = Modules.namaSemakAbWaran;
        ...
        public AkAnggarSemakController(
            ...
            string password)
        {
            ...
            List<AkAnggar> akAnggar = new List<AkAnggar>();
            ...
            if (dKonfigKelulusanId != null)
            {
                ...
                HttpContext.Session.SetInt32("DPenyemakId", (int)dKonfigKelulusanId);

                if (_unitOfWork.DKonfigKelulusanRepo.IsValidUser((int)dKonfigKelulusanId, password, EnJenisModulKelulusan.AnggaranHasil, EnKategoriKelulusan.Penyemak) == false)
                {
                ...
                else
                {
                    ...
                    akAnggar = _unitOfWork.AkAnggarRepo.GetResultsByDPekerjaIdFromDKonfigKelulusan(searchString, date1, date2, searchColumn, EnStatusBorang.Sah, (int)dKonfigKelulusanId, EnKategoriKelulusan.Penyemak, EnJenisModulKelulusan.AnggaranHasil);
        ...
        public async Task<IActionResult> Semak(int id, int dKonfigKelulusanId, string syscode)
        {
            ...
            if (akAnggar != null)
            {
                ...
                _unitOfWork.AkAnggarRepo.Semak(id, dKonfigKelulusanId, user?.UserName ?? "");

                _appLog.Insert("Posting", "Menyemak " + akAnggar.NoRujukan ?? "" + "; penyemakId: " + dKonfigKelulusanId.ToString(), akAnggar.NoRujukan ?? "", id, akAnggar.Jumlah, pekerjaId, modul, syscode, namamodul, user);
        ...
        public async Task<IActionResult> HantarSemulaAsync(int id, string? tindakan, string syscode)
        {
            ...
            if (akAnggar != null)
            {
                _unitOfWork.AkAnggarRepo.HantarSemula(id, tindakan, user?.UserName ?? "");

                _appLog.Insert("Ubah", "Hantar Semula " + akAnggar.NoRujukan ?? "", akAnggar.NoRujukan ?? "", id, akAnggar.Jumlah, pekerjaId, modul, syscode, namamodul, user);
                _context.SaveChanges();
        ...
        private void PopulateFormFields(string searchString, string password, string searchDate1, string searchDate2)
        {
            ...
            ViewBag.DKonfigKelulusan = _unitOfWork.DKonfigKelulusanRepo.GetResultsByCategoryGroupByDPekerja(EnKategoriKelulusan.Penyemak, EnJenisModulKelulusan.AnggaranHasil);
        }
